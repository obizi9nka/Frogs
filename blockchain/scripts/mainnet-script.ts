...
import Pool_json from "../artifacts/contracts/core/UniswapV3Pool.sol/UniswapV3Pool.json"
import ERC20_json from "../artifacts/@openzeppelin/contracts/token/ERC20/ERC20.sol/ERC20.json"
import PancakeFactory_json from '../artifacts/contracts/core/UniswapV3Factory.sol/UniswapV3Factory.json'
import Manager_json from '../artifacts/contracts/periphery/NonfungiblePositionManager.sol/NonfungiblePositionManager.json'
import Router_json from '../artifacts/contracts/periphery/SwapRouter.sol/SwapRouter.json'
...
export class Tokens {
    ...
    constructor(_usdt?: string, _usdc?: string, _busd?: string, _wbnb?: string) {
...
export class PancakeAddresses {
    ...
    constructor(_factory?: string, _router?: string, _manager?: string) {
...
export async function deploy(tokens: Tokens, pancakeAddresses: PancakeAddresses, fee: number, prefix: string) {
    ...
    const referal = await Referal.deploy(acct1.address, constants.mainnet_addressOfPrivateKey) as FrogReferal;
    await referal.deployed();
    // ==================
    const pancakeFactory = new ethers.Contract(pancakeAddresses.factory, PancakeFactory_json.abi, acct1) as UniswapV3Factory;


    const pool_busd_usdt = new ethers.Contract(await pancakeFactory.getPool(busd.address, usdt.address, fee), Pool_json.abi, acct1) as UniswapV3Pool
    // const pool_busd_usdc = new ethers.Contract(await pancakeFactory.getPool(busd.address, usdc.address, fee), Pool_json.abi, acct1) as UniswapV3Pool
    // const pool_usdt_usdc = new ethers.Contract(await pancakeFactory.getPool(usdt.address, usdc.address, fee), Pool_json.abi, acct1) as UniswapV3Pool
    ...
    const factory = await FrogFactory.deploy(referal.address, wbnb.address, pancakeFactory.address, acct1.address, router.address) as FrogFactory;
